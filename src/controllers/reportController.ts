...
class ReportController implements Controller {
  ...
  public upload_middleware = multer(
    { dest: './tmp/uploads/' },
  );
  ...
  private initializeRoutes() {
    ...
    this.router.post(`${this.path}/:id/picture`, this.upload_middleware.any('images'), this.uploadPicture);
  ...
  private checkAndMove = (files: any): { error: Array<Error>, paths: Array<string> } => {
    ...
    files.forEach((file: any) => {
      ...
      if (mimetype[0] === 'image') {
        const filename: string = `${file.filename}.${file.mimetype.split('/')[1]}`;
        const newPath: string = `${this.imagePath}${filename}`;
        paths.push(newPath);

        fs.rename(file.path, newPath, (err: Error) => {
          if (err) {
            error[error.length] = { name: 'rename', message: `Failed to rename: ${err}` };
          }
        });
      } else if (fs.existsSync(file.path)) {
        fs.unlink(file.path, (err: Error) => {
          if (err) {
            error[error.length] = { name: 'remove', message: `Failed to remove: ${err}` };
          }
        });
      }
    });
